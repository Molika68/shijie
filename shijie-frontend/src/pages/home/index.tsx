...
import { getAppConfig, getHistory, getUser, HistoryItem, isLoggedIn } from '../../services/api';
import { formatRelativeTime, getGreeting, getObjectEmoji } from '../../utils/time';
      ...
      <View className="home-header">
        ...
        {aiMock && (
          <View className="mock-banner">
            <Text className="mock-title">⚠️ 演示模式</Text>
            <Text className="mock-desc">
              当前未配置 AI，识物结果不准确。请在 backend/.env 设置 DASHSCOPE_API_KEY 后重启。
            </Text>
          </View>
        )}
        ...
        <View className="recognize-section">
          ...
          <View className="hint">支持拍照或从相册选择，图片将自动压缩后上传</View>
        </View>
      </View>
              ...
              <View key={item.id} className="card-item" onClick={() => handleItemClick(item)}>
                <View className="left">
                  <View className="thumb">
                    {item.inputImage ? (
                      <Image className="thumb-img" src={resolveAssetUrl(item.inputImage)} mode="aspectFill" />
                    ) : (
                      <Text>{getObjectEmoji(item.recognition?.object)}</Text>
                    )}
                  </View>
                  <View className="info">
                    <View className="name">{item.recognition?.object || '识别记录'}</View>
